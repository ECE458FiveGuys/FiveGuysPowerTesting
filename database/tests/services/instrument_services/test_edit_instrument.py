...
class EditInstrumentTestCase(TestCase):
    ...
    def test_edit_instrument_happy_case(self):
        ...
        EditInstrument(user_id=user.id, password=user.password, instrument_id=instrument.id, model_id=model.id, serial_number="serial_number_2").execute()
    ...
    def test_edit_instrument_invalid_model_id_exception(self):
        user, model, instrument = create_admin_and_model_and_instrument()
        try:
            EditInstrument(user_id=user.id, password=user.password, instrument_id=instrument.id, model_id=model.id + 1, serial_number="serial_number").execute()
    ...
    def test_edit_instrument_invalid_instrument_id_exception(self):
        user, model, instrument = create_admin_and_model_and_instrument()
        try:
            EditInstrument(user_id=user.id, password=user.password, instrument_id=instrument.id + 1, model_id=model.id, serial_number="serial_number").execute()
    ...
    def test_edit_instrument_without_required_fields_throws_exception(self):
        user, model, instrument = create_admin_and_model_and_instrument()
        try:
            EditInstrument(user_id=user.id, password=user.password, instrument_id=None, model_id=None, serial_number=None).execute()
    ...
    def test_edit_instrument_non_unique_throws_exception(self):
        user, model, instrument = create_admin_and_model_and_instrument()
        instrument2 = CreateInstrument(user_id=user.id, password=user.password, model_id=model.id, serial_number="serial_number_2").execute()
        try:
            EditInstrument(user_id=user.id, password=user.password, instrument_id=instrument2.id, model_id=model.id, serial_number="serial_number").execute()
